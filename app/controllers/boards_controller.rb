...
def sales
    ...
    @budget_monthy = Info.where(date: start_datem..end_datem).select("COALESCE(SUM(ord), 0) AS ord_sum, COALESCE(SUM(val), 0) AS val_sum, COALESCE(SUM(bud_ord), 0) AS bud_ord_sum, COALESCE(SUM(bud_val), 0) AS bud_val_sum, COALESCE(SUM(py_ord), 0) AS py_ord_sum, COALESCE(SUM(py_val), 0) AS py_val_sum").group(:id).first
